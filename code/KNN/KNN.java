...
public class KNN {
	...
	public static void loadTrainData(File file) throws IOException{
		...
		//System.out.println(trainAttrVals.get(0));
	}
	...
	public static void loadTestData(File testFile) throws IOException{
		...
		//System.out.println(testAttrVals.get(0));
	}
	...
	public static void calculateDistance(){
		...
		//for(int i = 0; i < 400; i++){
			...
			for(int j = 0; j < trainAttrVals.size(); j++){
				...
				arr.add(new ABC(j,trainClassVals.get(j),finalDist));
			}
	...
	public static void preditClassLabels(ArrayList<ABC> indexClassDistances){
		//K_NN;
		int zz = 0;
		Hashtable<Integer,Integer> interimClasses = new Hashtable<>();
		interimClasses.put(0, 0);
		interimClasses.put(1, 0);
		for(ABC ir : indexClassDistances){
